...
fn parse_llc(is_server: bool) -> impl FnMut(&[u8]) -> Result<(&[u8], &[u8]), nom::Err<Error>> {
    move |input| {
        let llc_header = if is_server {
            [0xE6, 0xE6, 0x00]
        } else {
            [0xE6, 0xE7, 0x00]
        };
        map_err(tag(llc_header)(input), Error::InvalidLlcHeader)
    }
}
...
impl<'a> HdlcFrame<'a> {
    fn parse_without_fcs(
        is_server: bool,
    ) -> impl FnMut(&'a [u8]) -> Result<(&'a [u8], HdlcFrame<'a>), nom::Err<Error>> {
        move |input| {
            ...
            let (input, information) = if length as usize - header.len() == 1 {
                ...
                let (input, _) = parse_llc(is_server)(input)?;
                let information_length = length as usize - header.len() - 7;
    ...
    fn parse_nom(
        ...
        move |input| {
            ...
            let (input, (frame, hdlc_frame)) = consumed(Self::parse_without_fcs(is_server))(input)?;
    ...
    pub fn parse(is_server: bool, input: &'a [u8]) -> Result<(&'a [u8], Self), Error> {
        Self::parse_nom(is_server)(input)
...
/// This method should return the complete information of unsegmented and segmented HdlcFrames
pub fn parse_segmented_information<'a, 'b>(
    frames: &'b [HdlcFrame<'a>],
) -> Result<(&'b [HdlcFrame<'a>], Cow<'a, [u8]>), Error> {
    if frames.len() == 0 {
        Err(Error::Incomplete(None))
    } else if !frames[0].segmented {
        Ok((&frames[1..], Cow::from(frames[0].information)))
    } else {
...
mod test {
    ...
    use crate::{calculate_fcs, parse_address, parse_frame_length, Error, HdlcAddress, HdlcFrame};
    ...
    fn test_parse() {
        ...
        full_parse(HdlcFrame::parse(true, &frame), expected);
        let demo_frame = HdlcFrame::parse(false, &DEMO_FRAME);
